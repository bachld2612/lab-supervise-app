...
import { create } from 'api/semester';
...
export default function AddSemester() {
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values) => {
      ...
      const response = await create(semesterData as Semester);

      if (response.statusCode === HttpStatusCode.Ok) {
        navigate('/semester', {
          state: { alert: { open: true, severity: 'success', message: 'Thêm học kỳ thành công' } }
        });
