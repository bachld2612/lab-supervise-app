...
export default function EditTeacher() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      // Fetch sections
      const sectionsRes = await getSections({ page: 0, size: 1000, status: '1' });
      if (sectionsRes.statusCode === HttpStatusCode.Ok) {
        setSections(sectionsRes.data.content);
      } else if (sectionsRes.statusCode === HttpStatusCode.Unauthorized) {
        logout();
        return;
      }
  ...
  const formik = useFormik<Teacher>({
    ...
    onSubmit: async (values) => {
      ...
      if (response.statusCode === HttpStatusCode.Ok) {
        navigate('/teacher', {
          state: { alert: { open: true, severity: 'success', message: 'Cập nhật giảng viên thành công' } }
        });
  ...
  return (
    <Box>
      <Box display="flex" alignItems="center" gap={2} mb={3}>
        <Typography variant="h3">Cập nhật giảng viên</Typography>
      </Box>
