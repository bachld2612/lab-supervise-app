  ...
  const formik = useFormik<Schedule>({
    ...
    onSubmit: async (values) => {
      ...
      if (response.statusCode === HttpStatusCode.Ok) {
        navigate('/schedule', {
          state: { alert: { open: true, severity: 'success', message: 'Thêm ca học thành công' } }
        });
  ...
  return (
    <Box>
      <Box display="flex" alignItems="center" gap={2} mb={3}>
        <Typography variant="h3">Thêm ca học</Typography>
      </Box>
          ...
          <Grid container spacing={2}>
            ...
            <Grid size={{ xs: 12 }}>
              ...
              <Autocomplete
                ...
                value={ALL_DAYS.filter((day) => formik.values.daysOfWeek.split(',').includes(day.value))}
                onChange={(_, newValue) => {
            ...
            <Grid size={{ xs: 12 }}>
              ...
              <Autocomplete
                ...
                options={ALL_PERIODS}
                getOptionLabel={(option) => option.label}
                value={ALL_PERIODS.filter((period) => formik.values.periods.split(',').includes(period.value))}
                onChange={(_, newValue) => {
                  const values = newValue.map((item) => item.value).join(',');
