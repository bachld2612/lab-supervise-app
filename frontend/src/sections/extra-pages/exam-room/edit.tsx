...
import { getById, update } from 'api/exam-room';
...
import {
  ...
  derivePeriodsFromTime,
  normalizePeriods,
  parsePeriodValues
} from 'sections/extra-pages/exam-room/form-helpers';

export default function EditExamRoom() {
  const intl = useIntl();
  ...
  const [initialFormValues, setInitialFormValues] = useState<ExamRoomFormValues>(emptyExamRoomValues);
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      if (id) {
        const resE = await getById(Number(id));
        if (resE.statusCode === HttpStatusCode.Ok) {
          const e: ExamRoom = resE.data;
          setInitialFormValues({
            code: e.code,
            roomId: e.roomId,
            teacher1Id: e.teacher1Id,
            teacher2Id: e.teacher2Id,
            subjectId: e.subjectId,
            semesterId: e.semesterId,
            maxStudent: e.maxStudent,
            examDate: e.examDate,
            periods: e.periods || derivePeriodsFromTime(e.startTime, e.endTime)
          });
        } else if (resE.statusCode === HttpStatusCode.Unauthorized) {
          logout();
        }
      }
      setLoading(false);
  ...
  const formik = useFormik<ExamRoomFormValues>({
    ...
    onSubmit: async (values) => {
      ...
      const response = await update(payload as Partial<ExamRoom>, Number(id));

      if (response.statusCode === HttpStatusCode.Ok) {
        navigate('/exam-room', {
          state: { alert: { open: true, severity: 'success', message: 'Cập nhật phòng thi thành công' } }
        });
  ...
  return (
    <Box>
      <Box display="flex" alignItems="center" gap={2} mb={3}>
        <Typography variant="h3">Cập nhật phòng thi</Typography>
      </Box>
          ...
          <Grid>
            <Grid size={12} sx={{ p: 0, m: 0 }}>
              <Stack direction="row" sx={{ justifyContent: 'flex-end', mt: 3 }}>
                ...
                <AnimateButton>
                  <Button variant="contained" type="submit" disabled={formik.isSubmitting} sx={{ ml: 1 }}>
                    Cập nhật
                  </Button>
                </AnimateButton>
              </Stack>
