...
import { getById, update } from 'api/subject';
...
export default function EditSubject() {
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values) => {
      const response = await update(values as unknown as Subject, Number(id));

      if (response.statusCode === HttpStatusCode.Ok) {
        navigate('/subject', {
          state: { alert: { open: true, severity: 'success', message: 'Cập nhật môn học thành công' } }
        });
  ...
  return (
    <Box>
      <Box display="flex" alignItems="center" gap={2} mb={3}>
        <Typography variant="h3">Cập nhật môn học</Typography>
      </Box>
