...
import { getById, update } from 'api/manageClass';
...
export default function EditManageClass() {
  const intl = useIntl();
  ...
  const [classData, setClassData] = useState<ManageClass>({
    id: 0,
    status: 1,
    name: '',
    maxStudent: 0,
    ...
    onSubmit: async (values) => {
      ...
      if (response.statusCode === HttpStatusCode.Ok) {
        navigate('/manage-class', {
          state: { alert: { open: true, severity: 'success', message: 'Cập nhật lớp thành công' } }
        });
  ...
  return (
    <Box>
      <Box display="flex" alignItems="center" gap={2} mb={3}>
        <Typography variant="h3">Cập nhật lớp quản lý</Typography>
      </Box>
