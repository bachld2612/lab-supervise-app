...
import { getById, update } from 'api/student';
...
export default function EditStudent() {
  ...
  const [initialStudent, setInitialStudent] = useState<Student>({
    id: 0,
    status: 1,
    phone: '',
    fullName: '',
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      // Fetch classes
      const classesRes = await getManageClasses({ page: 0, size: 1000, status: '1' });
      if (classesRes.statusCode === HttpStatusCode.Ok) {
        setClasses(classesRes.data.content);
      } else if (classesRes.statusCode === HttpStatusCode.Unauthorized) {
        logout();
        return;
      }
  ...
  const formik = useFormik<Student>({
    ...
    onSubmit: async (values) => {
      ...
      if (response.statusCode === HttpStatusCode.Ok) {
        navigate('/student', {
          state: { alert: { open: true, severity: 'success', message: 'Cập nhật sinh viên thành công' } }
        });
  ...
  return (
    <Box>
      <Box display="flex" alignItems="center" gap={2} mb={3}>
        <Typography variant="h3">Cập nhật sinh viên</Typography>
      </Box>
