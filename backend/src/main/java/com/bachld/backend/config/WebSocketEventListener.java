...
public class WebSocketEventListener {
  ...
  private void notifyClass(Student student, User user, Classes activeClass, String type) {
    StudentClassInfoResponse response =
        StudentClassInfoResponse.builder()
  ...
  private Classes findActiveClass(Integer studentId) {
    ...
    List<Classes> potentialClasses =
        studentClassRepository.findActiveClassesByStudentId(studentId, today);
  ...
  private boolean notifyExamRoomIfActive(Student student, User user, String type) {
    ...
    if (examRoom == null && "DISCONNECT".equals(type)) {
      ...
      Integer registeredRoomId =
          connectedExamStudentRegistry.getExamRoomForStudent(student.getId());
    ...
    studentExamRoomRepository
        .findByStudentIdAndExamRoomId(student.getId(), examRoom.getId())
        .ifPresent(
            ser -> {
