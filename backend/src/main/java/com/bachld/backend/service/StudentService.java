...
public class StudentService {
  ...
  public Page<StudentResponse> getList(
  ...
  public void create(StudentCreateRequest request) {
    ...
    manageClassRepository
        .findClassByIdAndStatus(request.getManageClassId(), Status.ACTIVE.getValue())
        .orElseThrow(
            () ->
                new IllegalArgumentException(
                    "Không tìm thấy lớp quản lý có id: " + request.getManageClassId()));
  ...
  public void update(StudentUpdateRequest request, int id) {
    Student student =
        studentRepository
            .findById(id)
            .orElseThrow(
                () -> new IllegalArgumentException("Không tìm thấy sinh viên có id: " + id));

    User user =
        userRepository
            .findByIdAndStatus(student.getUserId(), Status.ACTIVE.getValue())
            .orElseThrow(
                () ->
                    new IllegalArgumentException(
                        "Không tìm thấy người dùng có id: " + student.getUserId()));
    ...
    if (request.getBirthday() != null) {
      user.setBirthday(
          hasText(request.getBirthday()) ? LocalDate.parse(request.getBirthday()) : null);
    }
  ...
  public ResponseEntity<InputStreamResource> downloadStudentImportTemplate() throws IOException {
    org.springframework.core.io.ClassPathResource file =
        new org.springframework.core.io.ClassPathResource(
            "templates/download/student_import_template.xlsx");

    return ResponseEntity.ok()
        .header(
            HttpHeaders.CONTENT_DISPOSITION, "attachment; filename=student_import_template.xlsx")
        .contentType(
      ...
      for (int i = 1; i <= sheet.getLastRowNum(); i++) {
        ...
        try {
          ...
          ManageClass manageClass =
              manageClassRepository
                  .findClassByNameAndStatus(
                      importRequest.getManageClassName(), Status.ACTIVE.getValue())
                  .orElseThrow(
                      () ->
                          new IllegalArgumentException(
  ...
  private String buildDefaultPassword(String studentCode) {
    ...
    if (normalizedCode.length() < 3) {
      throw new IllegalArgumentException(
          "Mã sinh viên phải có ít nhất 3 ký tự để tạo mật khẩu mặc định");
    }
