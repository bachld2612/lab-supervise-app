...
public class IncidentReportService {
    ...
    public Page<IncidentReportResponse> getListForTeacher(Pageable pageable, String keyword, Integer status, Integer roomId) {
        if (keyword != null) {
            keyword = "%" + keyword.trim().toLowerCase() + "%";
        } else {
            keyword = "%%";
        }
        return incidentReportRepository.findAllByKeyword(pageable, keyword, status, roomId, util.getCurrentUser().getId());
    }

    public Page<IncidentReportResponse> getListForStudent(Pageable pageable, String keyword, Integer status) {
        if (keyword != null) {
            keyword = "%" + keyword.trim().toLowerCase() + "%";
        } else {
            keyword = "%%";
        }
        return incidentReportRepository.findAllByKeyword(pageable, keyword, status, null, util.getCurrentUser().getId());
    }
    ...
    public void createForStudent(IncidentReportStudentCreateRequest request) {
        ...
        List<ClassResponse> classes = classRepository.findActiveClassByStudentUserId(currentUser.getId(), today);
    ...
    private IncidentReport getAndValidatePending(Integer id) {
        IncidentReport report = incidentReportRepository.findById(id)
                .orElseThrow(() -> new IllegalArgumentException("Không tìm thấy báo cáo sự cố có id: " + id));
        if (report.getStatus() != 0) {
            throw new IllegalArgumentException("Không thể thao tác với báo cáo đã được xử lý hoặc từ chối");
        }
        return report;
    }
    ...
    private void validateOwner(IncidentReport report) {
        ...
        if (!report.getReporterId().equals(currentUser.getId())) {
            throw new ResponseStatusException(HttpStatus.FORBIDDEN, "Bạn không có quyền chỉnh sửa báo cáo này");
        }
    }
}