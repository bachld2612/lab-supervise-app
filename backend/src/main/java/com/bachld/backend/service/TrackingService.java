...
public class TrackingService {
  ...
  private static String normalize(String s) {
    if (s == null) return "";
    return Normalizer.normalize(s, Normalizer.Form.NFD).replaceAll("\\p{Mn}", "").toLowerCase();
  }
  ...
  @Transactional
  public StudentClassInfoResponse processTracking(
      Integer userId, StudentClassInfoCreateRequest request) {
    ...
    if (activeExam.isPresent()) {
      ...
      StudentExamRoom ser =
          studentExamRoomRepository
              .findByStudentIdAndExamRoomId(student.getId(), exam.getId())
              .orElse(null);
      ...
      if (trackingEnabled) {
        List<String> allowedApps =
            allowedApplicationRepository.findActiveAppNamesByExamRoomId(exam.getId());
    ...
    List<Classes> potentialClasses =
        studentClassRepository.findActiveClassesByStudentId(student.getId(), today);
    ...
    Map<Integer, Schedule> scheduleMap =
        allSchedules.stream().collect(Collectors.toMap(BaseEntity::getId, s -> s));
    ...
    StudentClass studentClass =
        studentClassRepository
            .findByStudentIdAndClassId(student.getId(), activeClass.getId())
            .orElse(null);
  ...
  @Transactional
  public StudentClassInfoResponse processClipboardEvent(
      Integer userId, ClipboardEventRequest request) {
    Student student = studentRepository.findByUserId(userId).orElse(null);
    if (student == null) {
      return null;
    }
    ...
    String clipboardText =
        clipboardTextCryptoService.decrypt(
    ...
    if (clipboardText.getBytes(java.nio.charset.StandardCharsets.UTF_8).length
        > MAX_CLIPBOARD_TEXT_BYTES) {
      throw new IllegalArgumentException(
          "Clipboard text exceeds " + MAX_CLIPBOARD_TEXT_BYTES + " bytes");
    }
    ...
    if (activeExam.isPresent()) {
      ...
      return buildClipboardResponse(
          exam.getId(), student, studentName, appName, action, clipboardText, "EXAM");
    }
    ...
    return buildClipboardResponse(
        activeClass.getId(), student, studentName, appName, action, clipboardText, null);
  }

  private StudentClassInfoResponse buildClipboardResponse(
      Integer contextId,
      Student student,
      String studentName,
      String appName,
  ...
  private boolean isBannedApplication(Integer classId, String appName) {
    ...
    return bannedApps.stream().anyMatch(banned -> normalizedApp.contains(normalize(banned)));
  }
}
