...
public class AuthService {
  ...
  public AuthResult login(LoginRequest loginRequest) {
    User user =
        userRepository
            .findByEmailAndStatus(loginRequest.getEmail(), Status.ACTIVE.getValue())
            .orElseThrow(
                () -> new IllegalArgumentException("Tài khoản hoặc mật khẩu không chính xác"));
    ...
    if (authentication.isAuthenticated()) {
      ...
      if (Objects.equals(loginRequest.getDevice(), "desktop")) {
        ...
        if (!hasActiveSchedule) {
          throw new IllegalArgumentException(
              "Xác minh vị trí không thành công. Vui lòng đảm bảo bạn đang trong khu vực lớp học.");
        }

        List<String> validSsids =
            classRepository.findActiveWifiSsidsByStudentUserId(user.getId(), today);
      ...
      LoginResponse response =
          LoginResponse.builder().token(token).user(user).role(roleResponse).build();
  ...
  /**
   * Exchange a valid, non-expired refresh token for a new access token. Throws if the token is
   * unknown or expired (expired tokens are deleted).
   */
  ...
  /**
   * Logout: delete the refresh token from DB and blacklist the access token (by jti) until its
   * natural expiry. Both arguments are best-effort.
   */
