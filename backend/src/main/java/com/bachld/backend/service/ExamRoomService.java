...
public class ExamRoomService {
  ...
  public Page<ExamRoomResponse> getList(
      Pageable pageable, String keyword, Integer semesterId, Integer status) {
  ...
  private void fillStudyStatus(List<ExamRoomResponse> rooms) {
    ...
    rooms.forEach(
        er -> {
          ...
          if (today.isBefore(er.getExamDate())) {
            er.setStudyStatus(0);
            return;
          }
          if (today.isAfter(er.getExamDate())) {
            er.setStudyStatus(2);
            return;
          }
          if (!now.isBefore(er.getStartTime()) && !now.isAfter(er.getEndTime())) {
            er.setStudyStatus(1);
            return;
          }
          if (now.isAfter(er.getEndTime())) {
            er.setStudyStatus(2);
            return;
          }
          er.setStudyStatus(0);
  ...
  public void create(ExamRoomCreateRequest request) {
    ...
    validateRoomConflict(
        request.getRoomId(), examDate, periodRange.startTime(), periodRange.endTime(), null);
    validateTeacherConflict(
        request.getTeacher1Id(), examDate, periodRange.startTime(), periodRange.endTime(), null);
    validateTeacherConflict(
        request.getTeacher2Id(), examDate, periodRange.startTime(), periodRange.endTime(), null);
  ...
  public void update(ExamRoomUpdateRequest request, Integer id) {
    ExamRoom examRoom =
        examRoomRepository
            .findById(id)
            .orElseThrow(
                () -> new IllegalArgumentException("Không tìm thấy phòng thi có id: " + id));
    ...
    LocalDate examDate =
        request.getExamDate() != null
            ? LocalDate.parse(request.getExamDate())
  ...
  public List<String> importStudents(Integer examRoomId, MultipartFile file) throws IOException {
    ...
    ExamRoom examRoom =
        examRoomRepository
            ...
            .orElseThrow(
                () ->
                    new IllegalArgumentException("Không tìm thấy phòng thi có id: " + examRoomId));
    ...
    try (Workbook workbook = new XSSFWorkbook(file.getInputStream())) {
      ...
      for (int i = 9; i <= sheet.getLastRowNum(); i++) {
        ...
        List<StudentExamRoom> conflicts =
            studentExamRoomRepository.findByStudentIdAndExamDate(
                student.getId(), examRoom.getExamDate());
        boolean hasConflict =
            conflicts.stream()
                .anyMatch(
                    ser -> {
                      ExamRoom other =
                          examRoomRepository.findById(ser.getExamRoomId()).orElse(null);
      ...
      long currentCount =
          studentExamRoomRepository.countByExamRoomIdAndStatus(
              examRoomId, Status.ACTIVE.getValue());
  ...
  public List<ClassStudentTrackingResponse> getTrackingByExamRoomId(Integer examRoomId) {
    ...
    List<StudentAppUsageRaw> rawUsage =
        studentExamRoomInfoRepository.findAppUsageByExamRoomIdAndDate(
            examRoomId, startOfDay, endOfDay);
  ...
  public int getStudyStatus(Integer examRoomId) {
    ...
    if (examRoom == null
        || examRoom.getExamDate() == null
        || examRoom.getStartTime() == null
        || examRoom.getEndTime() == null) return 0;
    LocalDate today = LocalDate.now();
  ...
  public Page<StudentResponse> getStudentsByExamRoomId(
      Integer examRoomId, Pageable pageable, String keyword) {
    keyword = (keyword != null) ? "%" + keyword.trim().toLowerCase() + "%" : "%%";
    return studentRepository.findByExamRoomId(pageable, examRoomId, keyword);
  }

  public Page<StudentResponse> getStudentsNotInExamRoom(
      Integer examRoomId, Pageable pageable, String keyword) {
    keyword = (keyword != null) ? "%" + keyword.trim().toLowerCase() + "%" : "%%";
    return studentRepository.findStudentsNotInExamRoom(
        pageable, examRoomId, keyword, Status.ACTIVE.getValue());
  }
  ...
  public void addStudentsToExamRoom(Integer examRoomId, List<Integer> studentIds) {
    ...
    long currentCount =
        studentExamRoomRepository.countByExamRoomIdAndStatus(examRoomId, Status.ACTIVE.getValue());
  ...
  /**
   * Rejects adding a student who, at this exam's date/time, would already be in a class session (a
   * class whose schedule covers the exam date + overlapping time) or another exam (same date +
   * overlapping time).
   */
  private void validateNoScheduleConflict(ExamRoom examRoom, List<Integer> studentIds) {
    ...
    for (Integer studentId : studentIds) {
      ...
      for (ClassScheduleView cls :
          studentClassRepository.findClassSchedulesByStudentId(studentId)) {
        if (cls.getStartDate() != null
            && cls.getEndDate() != null
            && !examDate.isBefore(cls.getStartDate())
            && !examDate.isAfter(cls.getEndDate())
