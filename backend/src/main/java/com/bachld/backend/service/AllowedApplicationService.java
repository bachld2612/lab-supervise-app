...
public class AllowedApplicationService {
  ...
  public Page<AllowedApplicationResponse> getList(
  ...
  public void update(AllowedApplicationUpdateRequest request, int id) {
    Teacher teacher = getCurrentTeacher();
    AllowedApplication entity =
        allowedApplicationRepository
            .findById(id)
            .orElseThrow(
                () -> new IllegalArgumentException("Không tìm thấy ứng dụng có id: " + id));
  ...
  private void broadcastWhitelistUpdate(Integer examRoomId) {
    List<AllowedApplicationResponse> updated =
        allowedApplicationRepository
            .findByExamRoomIdAndStatus(examRoomId, Status.ACTIVE.getValue())
            .stream()
            .map(
                a ->
                    new AllowedApplicationResponse(
                        a.getId(),
                        a.getExamRoomId(),
    ...
    WhitelistUpdateMessage msg =
        new WhitelistUpdateMessage("WHITELIST_UPDATE", examRoomId, updated);
  ...
  private void getExamRoomAndCheckTeacher(Integer examRoomId, Integer teacherId) {
    ...
    if (!teacherId.equals(examRoom.getTeacher1Id())
        && !teacherId.equals(examRoom.getTeacher2Id())) {
      throw new ResponseStatusException(
          HttpStatus.FORBIDDEN, "Bạn không có quyền thao tác với phòng thi này");
    }
  }
