...
public class VncWebSocketHandler extends BinaryWebSocketHandler {
  ...
  public void afterConnectionEstablished(WebSocketSession ws) throws Exception {
    ...
    try {
      ...
      Thread relay =
          new Thread(
              () -> proxyAndRelay(ws, vncSocket, wsQueue, handshakeDone), "vnc-" + ws.getId());
  ...
  private boolean performNoAuth(InputStream vncIn, OutputStream vncOut, byte[] types)
      throws IOException {
    ...
    if (!hasNone) {
      log.warn(
          "VNC server did not offer no-auth security type. Offered={}", Arrays.toString(types));
      return false;
    }
