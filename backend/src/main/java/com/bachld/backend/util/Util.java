...
public class Util {
  ...
  public User getCurrentUser() {
    ...
    return userRepository
        .findByIdAndStatus(Integer.valueOf(userId), Status.ACTIVE.getValue())
        .orElse(null);
  }
  ...
  public void validateSection(Integer sectionId) {
    SectionResponse section =
        sectionRepository.findByIdAndStatus(sectionId, Status.ACTIVE.getValue());
    if (section == null) {
  ...
  public void validateTeacherCode(String code, Integer teacherId) {
    Optional<Teacher> teacher =
        teacherRepository.findByCodeAndStatus(code, Status.ACTIVE.getValue());
  ...
  public void validateStudentCode(String code, Integer studentId) {
    Optional<Student> student =
        studentRepository.findByCodeAndStatus(code, Status.ACTIVE.getValue());
  ...
  public void validateSubjectCode(String code, Integer id) {
    Optional<Subject> subject =
        subjectRepository.findByCodeAndStatus(code, Status.ACTIVE.getValue());
  ...
  public void validateIpAddress(String ipAddress, Integer userId, Integer roleId) {
    Optional<PersonalComputer> pc =
        personalComputerRepository.findByIpAddressAndRoleId(ipAddress, roleId);
  ...
  public void validateImportTemplate(Sheet sheet, String[] expectedHeaders) {
    ...
    if (headerRow == null) {
      throw new IllegalArgumentException(
          "File không đúng định dạng template: không tìm thấy hàng tiêu đề");
    }
    ...
    for (int i = 0; i < expectedHeaders.length; i++) {
      ...
      if (!expectedHeaders[i].equals(value)) {
        throw new IllegalArgumentException(
            "File không đúng định dạng template: cột "
                + (i + 1)
                + " phải là '"
                + expectedHeaders[i]
                + "'");
  ...
  /**
   * Kiểm tra phòng học có bị trùng lịch với lớp khác không. Conflict khi: cùng roomId + khoảng ngày
   * overlap + chia sẻ ít nhất 1 ngày trong tuần + giờ học overlap.
   *
  ...
  public void validateRoom(
    ...
    List<Classes> sameRoomClasses =
        classRepository.findByRoomIdAndStatus(roomId, Status.ACTIVE.getValue());
    ...
    for (Classes c : sameRoomClasses) {
      ...
      boolean dateOverlap =
          !startDate.isAfter(c.getEndDate()) && !endDate.isBefore(c.getStartDate());
      ...
      Set<String> otherDays =
          new HashSet<>(Arrays.asList(otherSchedule.getDaysOfWeek().split(",")));
      ...
      boolean timeOverlap =
          newSchedule.getStartTime().isBefore(otherSchedule.getEndTime())
