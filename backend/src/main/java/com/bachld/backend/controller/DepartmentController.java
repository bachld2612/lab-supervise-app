...
public class DepartmentController {
    ...
    public ResponseEntity<?> getList(
        ...
        return ResponseEntity.ok(new BaseResponse<>(HttpStatus.OK.value(), departmentService.getList(pageable, keyword, status)));
    }

    @GetMapping("/v1/{id}")
    @AuthFilter(role = "ADMIN")
    public ResponseEntity<?> getById (@PathVariable int id) {
        return ResponseEntity.ok(new BaseResponse<>(HttpStatus.OK.value(), departmentService.getById(id)));
    }
    ...
    @PutMapping("/v1/{id}")
    @AuthFilter(role = "ADMIN")
    public ResponseEntity<?> update(@RequestBody @Valid DepartmentUpdateRequest request, @PathVariable int id) {
