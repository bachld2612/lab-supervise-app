...
public class ExamRoomController {
  ...
  public ResponseEntity<?> getList(
    ...
    return ResponseEntity.ok(
        new BaseResponse<>(
            HttpStatus.OK.value(), examRoomService.getList(pageable, keyword, semesterId, status)));
  }
  ...
  public ResponseEntity<?> getStudentExamRooms() {
    return ResponseEntity.ok(
        new BaseResponse<>(HttpStatus.OK.value(), examRoomService.getStudentExamRooms()));
  }
  ...
  public ResponseEntity<?> getTeacherExamRooms() {
    return ResponseEntity.ok(
        new BaseResponse<>(HttpStatus.OK.value(), examRoomService.getTeacherExamRooms()));
  }
  ...
  public ResponseEntity<?> getById(@PathVariable int id) {
    return ResponseEntity.ok(
        new BaseResponse<>(HttpStatus.OK.value(), examRoomService.getById(id)));
  }
  ...
  public ResponseEntity<?> update(
      @RequestBody @Valid ExamRoomUpdateRequest request, @PathVariable int id) {
  ...
  public ResponseEntity<?> importStudents(
      @PathVariable int examRoomId, @RequestParam("file") MultipartFile file) throws IOException {
    return ResponseEntity.ok(
        new BaseResponse<>(
            HttpStatus.OK.value(), examRoomService.importStudents(examRoomId, file)));
  }
  ...
  @AuthFilter(role = "ADMIN")
  public ResponseEntity<?> addStudentsToExamRoom(
      @PathVariable int examRoomId, @RequestBody StudentExamRoomRequest request) {
  ...
  @AuthFilter(role = "ADMIN")
  public ResponseEntity<?> removeStudentsFromExamRoom(
      @PathVariable int examRoomId, @RequestBody StudentExamRoomRequest request) {
  ...
  public ResponseEntity<?> getTracking(@PathVariable int examRoomId) {
    return ResponseEntity.ok(
        new BaseResponse<>(
            HttpStatus.OK.value(), examRoomService.getTrackingByExamRoomId(examRoomId)));
  }
  ...
  public ResponseEntity<?> getConnectedStudents(@PathVariable int examRoomId) {
    return ResponseEntity.ok(
        new BaseResponse<>(
            HttpStatus.OK.value(), examRoomService.getConnectedStudents(examRoomId)));
  }
  ...
  public ResponseEntity<?> getStudyStatus(@PathVariable int examRoomId) {
    return ResponseEntity.ok(
        new BaseResponse<>(HttpStatus.OK.value(), examRoomService.getStudyStatus(examRoomId)));
  }

  @PutMapping("/v1/{examRoomId}/tracking-enabled")
  @AuthFilter(role = "TEACHER")
  public ResponseEntity<?> setTrackingEnabled(
      @PathVariable int examRoomId, @RequestParam boolean enabled) {
  ...
  public ResponseEntity<?> generateWifiSsid(@PathVariable int id) {
    return ResponseEntity.ok(
        new BaseResponse<>(HttpStatus.OK.value(), examRoomService.generateWifiSsid(id)));
  }
}
