...
public class PersonalComputerController {
  ...
  public ResponseEntity<?> getByUserId() {
    return ResponseEntity.ok(
        new BaseResponse<>(HttpStatus.OK.value(), personalComputerService.getByUserId()));
  }
  ...
  public ResponseEntity<?> getStudentsByClassId(@PathVariable Integer classId) {
    return ResponseEntity.ok(
        new BaseResponse<>(
            HttpStatus.OK.value(), personalComputerService.getStudentsByClassId(classId)));
  }
  ...
  public ResponseEntity<?> getStudentsByExamRoomId(@PathVariable Integer examRoomId) {
    return ResponseEntity.ok(
        new BaseResponse<>(
            HttpStatus.OK.value(), personalComputerService.getStudentsByExamRoomId(examRoomId)));
  }

  @PutMapping("/v1/student/{userId}")
  @AuthFilter(role = "TEACHER,IT_CENTER")
  public ResponseEntity<?> updateStudentPc(
      @PathVariable Integer userId, @RequestBody @Valid PersonalComputerUpdateRequest request) {
