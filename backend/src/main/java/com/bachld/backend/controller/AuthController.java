...
public class AuthController {
  ...
  public ResponseEntity<?> login(@RequestBody @Valid LoginRequest loginRequest) {
    ...
    ResponseCookie cookie =
        buildRefreshCookie(result.getRefreshToken(), Duration.ofDays(REFRESH_TOKEN_EXPIRED_DAY));
  ...
  public ResponseEntity<?> refresh(
    ...
    return ResponseEntity.ok(
        new BaseResponse<>(HttpStatus.OK.value(), new TokenResponse(accessToken)));
  }
  ...
  public ResponseEntity<?> getValidWifiSsids(@RequestParam String email) {
    return ResponseEntity.ok(
        new BaseResponse<>(HttpStatus.OK.value(), authService.getValidWifiSsids(email)));
  }
